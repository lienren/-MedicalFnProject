...
export default {
  ...
  methods: {
      ...
      this.actionButtons = [
        {
          ...
          click: async (e) => {
            this.uploadSubImgFileList = []
            let result = await api.getPlayParentChildDetail({
              id: e.id
            })
        ...
        },
        {
          ...
          confirm: {
            title: '确认要删除吗？',
            confirm: async (e) => {
              let result = await api.delPlayParentChild({
                id: e.id
              })
              if (result) {
                this.$message.success('亲子汇删除成功！')
                this.fetch({
                  ...this.pagination
                })
              }
    ...
    async fetch (param = {}) {
      let result = await api.getPlayParentChild({
        ...param
      })
      this.data = []
      if (result) {
    ...
    async addOrupdateAction () {
      ...
      let result
      if (this.info.id > 0) {
        result = await api.editPlayParentChild({
          ...this.info
        })
      } else {
        result = await api.addPlayParentChild({
          ...this.info
        })
      }
