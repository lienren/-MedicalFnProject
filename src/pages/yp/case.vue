...
export default {
  ...
  methods: {
      ...
      this.actionButtons = [
        {
          ...
          click: async (e) => {
            this.uploadSubImgFileList = []
            let result = await api.getPlayCaseDetail({
              id: e.id
            })
        ...
        },
        {
          ...
          confirm: {
            title: '确认要删除吗？',
            confirm: async (e) => {
              let result = await api.delPlayCase({
                id: e.id
              })
              if (result) {
    ...
    async fetch (param = {}) {
      let result = await api.getPlayCase({
        ...param
      })
      this.data = []
      if (result) {
    ...
    async addOrupdateAction () {
      ...
      let result
      if (this.info.id > 0) {
        result = await api.editPlayCase({
          ...this.info
        })
      } else {
        result = await api.addPlayCase({
          ...this.info
        })
      }
