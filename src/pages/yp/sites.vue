    ...
    <a-modal style="top: 20px;" width="80%" :title="cuTitle" v-model="cuVisible" @ok="addOrupdateAction" :okText="cuConfirmText" cancelText="取消" :confirmLoading="cuConfirmLoading">
      <a-form>
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='地址'>
          <a-input placeholder='请输入地址' v-model="info.address">
          </a-input>
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='营业时间'>
          <a-input placeholder='请输入营业时间' v-model="info.workTime">
          </a-input>
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='开业时间'>
          <a-input placeholder='请输入开业时间' v-model="info.openTime">
          </a-input>
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='面积'>
          <a-input-number :min="1" v-model="info.size" />平米
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='高度'>
          <a-input-number :min="1" v-model="info.height" />米
        </a-form-item>
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label='可容纳人数'>
...
export default {
  ...
  methods: {
      ...
      this.actionButtons = [
        ...
        },
        {
          ...
          confirm: {
            title: '确认要删除吗？',
            confirm: async (e) => {
              let result = await api.delPlaySite({
                id: e.id
              })
              if (result) {
    ...
    async fetch (param = {}) {
      let result = await api.getPlaySite({
        ...param
      })
      this.data = []
      if (result) {
