        ...
        },
        {
          ...
          click: () => {
            if (this.data.length > 0) {
              let output = [['订单号', '客户姓名', '客户电话', '客户性别', '客户身份证', '客户渠道', '客户来源', '中介名称', '中介联系人', '中介联系电话', '贷款金额', '贷款利息', '贷款本金', '贷款时间', '还款时间', '审核员', '备注', '订单状态', '最终贷款金额', '最终贷款利息', '最终贷款本金', '展期总次数', '展期总金额', '最终还款时间', '实际还款金额', '实际还款时间', '最终备注', '下单时间', '最后更新时间']]
              this.data.forEach(d => {
                output.push([d.orderSn, d.userName, d.userPhone, d.userSex, d.userIdCard, d.userChannel, d.userSource, d.userSource === '中介' ? d.userSourceName : '', d.userSource === '中介' ? d.userSourceUserName : '', d.userSource === '中介' ? d.userSourcePhone : '', d.loanPrice, d.loanInterest, d.loanServicePrice, d.loanTime, d.shouldReturnTime, d.managerName, d.remark, d.stateName, d.lastloanPrice, d.lastloanInterest, d.lastloanServicePrice, d.extCount, d.extPrice, d.extReturnTime, d.state === 3 ? d.realReturnPrice : '', d.state === 3 ? d.realReturnTime : '', d.lastRemark, d.addTime, d.updateTime])
              })
              console.log('output:', output)
              this.$utils.Excel.exportExcel(output, '订单数据', '订单数据')
            } else {
